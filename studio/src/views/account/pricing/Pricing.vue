<template>
  <section v-if="Object.keys(pricing).length" id="pricing-plan">
    ...
    <div class="text-center">
      <h1 class="mt-5">{{ t("Pricing Plans") }}</h1>
      <p class="mb-2 pb-75">
        {{
          t(
            "No hidden costs, ugprade or downgrade any time. Choose the best plan to fit your needs."
          )
        }}
      </p>
      <div class="d-flex align-items-center justify-content-center mb-5 pb-50">
        <h6 class="mr-1 mb-0">{{ t("Monthly") }}</h6>
        <b-form-checkbox
          id="priceSwitch"
          v-model="status"
          name="price-switch"
          ...
          @input="tooglePlan"
        />
        <h6 class="ml-50 mb-0">{{ t("Annually") }}</h6>
      </div>
    </div>
    <!--/ title text and switch button -->
          ...
          <div class="pricing-trial-content d-flex justify-content-between">
            <div class="text-center text-md-left mt-3">
              <h3 class="text-primary">
                {{ t("Still not convinced? Start with a 30-day FREE trial!") }}
              </h3>
              <h5>
                {{
                  t("You will get full access to all the features for 30 days.")
                }}
              </h5>
              <b-button
                v-ripple.400="'rgba(255, 255, 255, 0.15)'"
